...
void GCodeGrbl::sendControllerReset()
{
    clearToHome();
...
void GCodeGrbl::sendGcode(QString line)
{
...
void GCodeGrbl::sendGcodeAndGetResult(int id, QString line)
{
    QString result;
...
bool GCodeGrbl::sendGcodeLocal(QString line, bool recordResponseOnFail /* = false */, int waitSec /* = -1 */, bool aggressive /* = false */, int currLine /* = 0 */)
{
    QString result;
    sendMsg("");
    resetState.set(false);
...
bool GCodeGrbl::checkGrbl(const QString& result)
{
...
bool GCodeGrbl::sendGcodeInternal(QString line, QString& result, bool recordResponseOnFail, int waitSec, bool aggressive, int currLine /* = 0 */)
{
    if (!port.isPortOpen())
    {
...
bool GCodeGrbl::waitForOk(QString& result, int waitSec, bool sentReqForLocation, bool sentReqForParserState, bool aggressive, bool finalize)
{
    int okcount = 0;
...
bool GCodeGrbl::waitForStartupBanner(QString& result, int waitSec, bool failOnNoFound)
{
    char tmp[BUF_SIZE + 1] = {0};
    int count = 0;
    int waitCount = waitSec * 10;// multiplier depends on sleep values below
    ...
    while (!resetState.get())
    {
        ...
        else
        {
            ...
            std::cout << "Mierda gorda 1" << std::endl;
            tmp[n] = 0;
            result.append(tmp);
...
void GCodeGrbl::parseCoordinates(const QString& received, bool aggressive)
{
    if (aggressive)
    {
...
void GCodeGrbl::sendStatusList(QStringList& listToSend)
{
...
QString GCodeGrbl::removeUnsupportedCommands(QString line)
{
...
QString GCodeGrbl::reducePrecision(QString line)
{
...
bool GCodeGrbl::isGCommandValid(float value, bool& toEndOfLine)
{
    toEndOfLine = false;
...
bool GCodeGrbl::isMCommandValid(float value)
{
    // supported values obtained from https://github.com/grbl/grbl/wiki
...
QStringList GCodeGrbl::doZRateLimit(QString inputLine, QString& msg, bool& xyRateSet)
{
    // i.e.
    //G00 Z1 => G01 Z1 F100
...
QString GCodeGrbl::getMoveAmountFromString(QString prefix, QString item)
{
...
void GCodeGrbl::axisAdj(char axis, float coord, bool inv, bool absoluteAfterAxisAdj, int sZC)
{
    if (inv)
    {
        coord = (-coord);
...
bool GCodeGrbl::SendJog(QString cmd, bool absoluteAfterAxisAdj)
{
    pollPosWaitForIdle(false);
...
void GCodeGrbl::setResponseWait(ControlParams controlParamsIn)
{
...
int GCodeGrbl::getSettingsItemCount()
{
    return settingsItemCount.get();
}

// 0.8c and above only!
void GCodeGrbl::checkAndSetCorrectMeasurementUnits()
{
...
void GCodeGrbl::setOldFormatMeasurementUnitControl()
{
    if (controlParams.useMm)
        sendGcodeLocal("G21");
    else
...
void GCodeGrbl::setConfigureMmMode(bool setGrblUnits)
{
    sendGcodeLocal("$13=0");
    if (setGrblUnits)
        sendGcodeLocal("G21");
...
void GCodeGrbl::clearToHome()
{
    maxZ = 0;
    motionOccurred = false;
}

int GCodeGrbl::getNumaxis()
{
	return numaxis;
}
...
{
    Q_UNUSED(levelingAlgorithm) Q_UNUSED(rect) Q_UNUSED(ySteps) Q_UNUSED(zStarting) Q_UNUSED(speed) Q_UNUSED(zSafe) Q_UNUSED(offset)
